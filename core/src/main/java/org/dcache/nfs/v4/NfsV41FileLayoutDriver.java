...
/**
 * layout driver for NFSv4.1 file layout type as defined in <a href="https://www.ietf.org/rfc/rfc5661.txt">rfc5661</a>
 */
public class NfsV41FileLayoutDriver implements LayoutDriver {
    ...
    public layout_content4 getLayoutContent(stateid4 stateid, int stripeSize, nfs_fh4 fh, deviceid4... deviceids)
         ...
         *
         * Zero. This means that filehandles used for each data server are the same as the filehandle returned by the
         * OPEN operation from the metadata server.
         *
         * One. This means that every data server uses the same filehandle: what is specified in nfl_fh_list[0].
         *
         * The same number of elements in nflda_multipath_ds_list. Thus, in this case, when sending an I/O operation to
         * any data server in nflda_multipath_ds_list[X], the filehandle in nfl_fh_list[X] MUST be used.
         */
