...
public class EXCHANGE_ID4resok implements XdrAble {
    ...
    public void xdrEncode(XdrEncodingStream xdr)
        ...
        {
            int $size = eir_server_impl_id.length;
            xdr.xdrEncodeInt($size);
            for (int $idx = 0; $idx < $size; ++$idx) {
                eir_server_impl_id[$idx].xdrEncode(xdr);
            }
        }
    }
    ...
    public void xdrDecode(XdrDecodingStream xdr)
        ...
        eir_server_scope = xdr.xdrDecodeDynamicOpaque();
        {
            ...
            eir_server_impl_id = new nfs_impl_id4[$size];
            for (int $idx = 0; $idx < $size; ++$idx) {
                eir_server_impl_id[$idx] = new nfs_impl_id4(xdr);
            }
        }
    }
